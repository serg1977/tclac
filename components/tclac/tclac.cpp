...
* Доработки и логи для AUTO setpoint: 2025–2026 (Grok-assisted)
**/
#include "esphome.h"
...
void tclacClimate::readData() {
  ...
  ESP_LOGD("TCL", "Reported target from AC: %.0f °C (you requested: %d °C)",
           target_temperature, 31 - target_temperature_set);
...
void tclacClimate::control(const ClimateCall &call) {
  ...
  if (call.get_target_temperature().has_value()) {
    ...
    if (switch_climate_mode == climate::CLIMATE_MODE_AUTO) {
      ESP_LOGI("TCL", "AUTO mode → target temperature изменена на %d °C (AC реагирует на setpoint)", requested);
    } else {
      ESP_LOGD("TCL", "Target temperature установлена на %d °C", requested);
    }
  } else {
